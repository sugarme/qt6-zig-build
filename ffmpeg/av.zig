...
/// Prefer `FormatContext.alloc_output_context2`.
pub extern fn avformat_alloc_output_context2(ctx: *?*FormatContext, oformat: ?*const OutputFormat, format_name: ?[*:0]const u8, filename: ?[*:0]const u8) c_int;
/// Prefer `FormatContext.new_stream`.
pub extern fn avformat_new_stream(s: *FormatContext, c: ?*const Codec) ?*Stream;
/// Prefer `FormatContext.write_header`.
pub extern fn avformat_write_header(s: *FormatContext, options: ?*Dictionary.Mutable) c_int;
/// Prefer `FormatContext.interleaved_write_frame`.
pub extern fn av_interleaved_write_frame(s: *FormatContext, pkt: ?*Packet) c_int;
/// Prefer `FormatContext.write_trailer`.
pub extern fn av_write_trailer(s: *FormatContext) c_int;
...
/// Prefer `IOContext.open`.
pub extern fn avio_open(s: *?*IOContext, url: [*:0]const u8, flags: c_int) c_int;
/// Prefer `IOContext.closep`.
pub extern fn avio_closep(s: *?*IOContext) c_int;
...
/// Register all compiled-in device drivers (call before av_find_input_format for devices).
pub extern fn avdevice_register_all() void;

/// Find an input format by its short name (e.g. "alsa", "pulse", "dshow").
pub extern fn av_find_input_format(short_name: [*:0]const u8) ?*const InputFormat;
...
/// Prefer `Codec.Context.send_frame`.
pub extern fn avcodec_send_frame(avctx: *Codec.Context, frame: ?*const Frame) c_int;
/// Prefer `Codec.Context.receive_packet`.
pub extern fn avcodec_receive_packet(avctx: *Codec.Context, avpkt: *Packet) c_int;
/// Prefer `Codec.Parameters.from_context`.
pub extern fn avcodec_parameters_from_context(par: *Codec.Parameters, codec: *const Codec.Context) c_int;
...
/// Prefer `Packet.rescale_ts`.
pub extern fn av_packet_rescale_ts(pkt: *Packet, tb_src: Rational, tb_dst: Rational) void;
...
/// Prefer `Frame.get_buffer`.
pub extern fn av_frame_get_buffer(frame: *Frame, @"align": c_int) c_int;
/// Prefer `Frame.make_writable`.
pub extern fn av_frame_make_writable(frame: *Frame) c_int;
...
pub const FormatContext = extern struct {
    ...
    pub fn alloc_output_context2(
        format_name: ?[*:0]const u8,
        filename: ?[*:0]const u8,
    ) Error!*FormatContext {
        var ctx: ?*FormatContext = null;
        _ = try wrap(avformat_alloc_output_context2(&ctx, null, format_name, filename));
        return ctx orelse error.FFmpegInvalid;
    }

    /// Add a new stream to a media file.
    pub fn new_stream(s: *FormatContext, c: ?*const Codec) error{OutOfMemory}!*Stream {
        return avformat_new_stream(s, c) orelse error.OutOfMemory;
    }

    /// Write the file header.
    pub fn write_header(s: *FormatContext, options: ?*Dictionary.Mutable) Error!void {
        _ = try wrap(avformat_write_header(s, options));
    }

    /// Write a packet to an output media file ensuring correct interleaving.
    pub fn interleaved_write_frame(s: *FormatContext, pkt: ?*Packet) Error!void {
        _ = try wrap(av_interleaved_write_frame(s, pkt));
    }

    /// Write the stream trailer to an output media file and free the file private data.
    pub fn write_trailer(s: *FormatContext) Error!void {
        _ = try wrap(av_write_trailer(s));
    }
};
...
pub const IOContext = extern struct {
    ...
    /// Open a file and create an IOContext for it.
    pub fn open(url: [*:0]const u8, flags: c_int) Error!*IOContext {
        var pb: ?*IOContext = null;
        _ = try wrap(avio_open(&pb, url, flags));
        return pb orelse error.FFmpegInvalid;
    }

    /// Close the resource accessed by the IOContext and free it, setting the pointer to NULL.
    pub fn closep(s: *?*IOContext) Error!void {
        _ = try wrap(avio_closep(s));
    }
};
...
pub const Packet = extern struct {
    ...
    pub fn rescale_ts(pkt: *Packet, tb_src: Rational, tb_dst: Rational) void {
...
pub const Codec = extern struct {
    ...
    pub const Parameters = extern struct {
        ...
        pub fn from_context(par: *Parameters, codec: *const Context) Error!void {
            _ = try wrap(avcodec_parameters_from_context(par, codec));
        }
    };
    ...
    pub const Context = extern struct {
        ...
        ///
        /// Send NULL to flush the encoder and get remaining packets.
        pub fn send_frame(cc: *Context, frame: ?*const Frame) Error!void {
            _ = try wrap(avcodec_send_frame(cc, frame));
        }

        /// Read encoded data from the encoder.
        pub fn receive_packet(cc: *Context, pkt: *Packet) Error!void {
            _ = try wrap(avcodec_receive_packet(cc, pkt));
        }
    };
...
pub const Frame = extern struct {
    ...
    ///
    /// The frame must have format, width, height (video) or nb_samples,
    /// ch_layout, format (audio) set before calling this function.
    pub fn get_buffer(frame: *Frame, alignment: c_int) Error!void {
        _ = try wrap(av_frame_get_buffer(frame, alignment));
    }

    /// Ensure that the frame data is writable.
    ///
    /// If the frame is already writable, this does nothing. Otherwise,
    /// a copy is made.
    pub fn make_writable(frame: *Frame) Error!void {
        _ = try wrap(av_frame_make_writable(frame));
    }
};
